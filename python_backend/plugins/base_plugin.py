...
class BasePlugin(IPlugin):
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            self._time_column = self._get_time_column()
            ...
            self.logger.info(
                f"Plugin loaded successfully: {len(self._data)} rows, {len(self._signals)} signals"
            )
    ...
    def get_data_range(
        self, signal: str, start_time: float, end_time: float
    ) -> Optional[Union[TemporalSignal, SpatialSignal]]:
    ...
    def _get_time_column(self) -> Optional[str]:
        """Get the name of the time column, cached after load."""
    ...
    def _get_spatial_range(
        self, data: pd.DataFrame, signal: str
    ) -> Optional[SpatialSignal]:
